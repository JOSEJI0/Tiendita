...
export const Cart = ({isOpen, onClose, cart, updateQuantity, removeFromCart, clearCart,
...
return(
    <div className="fixed inset-0 z-50 overflow-hidden">
      <div 
        className="absolute inset-0 bg-green-950/45 backdrop-blur-sm transition-opacity" 
        onClick={onClose} 
      />

      <div className="absolute inset-y-0 right-0 max-w-full flex pl-10">
        <div className="w-screen max-w-md bg-white shadow-md flex flex-col">
          <div className="px-6 py-5 bg-green-800 text-white flex items-center justify-between">
            <h2 className="text-lg font-bold flex items-center gap-2">
              <ShoppingBasket className="w-5 h-5 text-green-200" /> Mi Carrito
            </h2>
            <button 
              onClick={onClose}
              className="p-1.5 rounded-full hover:bg-green-700 transition-colors text-white cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
